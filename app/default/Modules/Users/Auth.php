...
class Auth
{
    ...
    private static function getUser($data)
    {
        ...
        if (!empty($email)) {
            return UserGateway::instance()->whereBy('email', $email)->first();
        }
    ...
    public static function user()
    {
        self::init();
        if (self::$userId) {
            $user = UserGateway::instance()->find(self::$userId);
    ...
    public static function logout()
    {
        ...
        if (empty($pnid)) {
            ...
            AuthGateway::instance()
                ->whereId(self::authId($pnid, $userAgent, self::clientIp()))
                ->delete();
        }
        self::$userId = 0;
    }
    ...
    public static function init()
    {
        ...
        $userAgent = self::userAgent();
        $ip = self::clientIp();
        $authId = self::authId($pnid, $userAgent, $ip);
    ...
    public static function validateUserId($userId)
    {
        ...
        $userId = UserGateway::instance()->whereBy('id', $userId)->id();
        if ($userId) {
            return true;
        }
