...
class CSVImportReader implements AbstractImportReader
{
    ...
    public function header()
    {
        ...
        foreach ($this->header as $index => $item) {
            if (empty($item)) {
                $item = 'R' . $this->headerRow . 'C' . ($index + 1);
            }
            $header[] = $item;
        }
        return $this->header;
    ...
    private function read()
    {
        ...
        if ($this->charset !== 'utf8' && is_array($this->rowBuffer)) {
            array_walk($this->rowBuffer, function(&$item) {
                $item = iconv($this->charset, 'utf8', $item);
            });
        }
    ...
    private function readHeader()
    {
        ...
        if ($this->charset !== 'utf8' && is_array($this->header)) {
            array_walk($this->header, function(&$item) {
                $item = iconv($this->charset, 'utf8', $item);
            });
        }
        $this->index ++;
