        ...
        <div class="images select-images">
            {if $items}
                {foreach from=$items item=i}
                    {strip}
                        <div class="image thumbnail" style="background-image:url('{media_url storage=$i.storage path=$i.path}');">
                            <a class="remove action-remove" href="#"><i class="glyphicon glyphicon-remove"></i></a>

                            <input type="hidden" name="media_ids[]" value="{$i.id}" />
                            <a class="image-center" href="{media_url storage=$i.storage path=$i.path}" target="blank"><span>{$i.width}x{$i.height}</span></a>
                        </div>
                    {/strip}
                {/foreach}                
            {/if}
                ...
                return {
                    upload: function () {
                        ...
                        $(uploadInput).fileupload({
                            ...
                            fail: function (e, data, test) {
                                console.log(e, data);
                                if (options.onError) {
                                    options.onError(data.jqXHR.responseJSON);
                                }
                            },
